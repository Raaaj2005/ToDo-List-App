...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className='mx-3 md:mx-auto my-5 rounded-xl bg-violet-100 p-5 md:w-[40%] min-h-[80vh]'>
        <h1 className='font-bold text-center text-3xl'>Taskley - Task Management Made Easy</h1>
        <div className="addTodo my-5 flex flex-col gap-4">
          <h2 className='text-xl font-bold'>Add a Todo</h2>
          <div className="flex">
            <input onChange={handleChange} value={todo} className="bg-white w-full rounded-full px-5 py-1" type="text" />
            <button onClick={handleAdd} disabled={todo.length<=3} className="bg-violet-800 hover:bg-violet-950 disabled:bg-violet-700 p-4 py-2 text-white rounded-full text-sm font-bold mx-2">Save</button>
          </div>
        </div>
